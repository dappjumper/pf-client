<template>
  <img SameSite="None" :src="avatarUrl">
</template>
        ...
        .then((result) => {
          if (result.data.ok) {
            this.externalQuery({
              ...
              .catch((error) => {
                console.log('Could not get file path', error)
              })
          }
        })
        .catch((error) => {
